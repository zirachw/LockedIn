...
llama.cpp's conventions are built around portability and quantization:
...
Running a quantized model looks like this from the command line.
...
Ollama's conventions are built around ease of use on top of llama.cpp's engine:
...
Running and calling a model looks like this.
...
vLLM's conventions are built around maximizing GPU throughput under concurrent load:
...
Serving a model looks like this.
...
vLLM's throughput under concurrent load far exceeds llama.cpp or Ollama, but it needs a real GPU to run at all, and its memory-management sophistication is solving a problem, serving hundreds of simultaneous users, that a single local developer running one model at a time simply does not have.
